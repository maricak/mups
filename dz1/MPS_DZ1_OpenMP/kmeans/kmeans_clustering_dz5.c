...
#pragma omp parallel for default(none), private(i), shared(npts)
    for (i=0; i<npts; i++) {
        float dist;
                          ...
                          int    *membership) /* out: [npoints] */
{
	...
	for (i = 0; i < npoints; i++) {
		omp_init_lock(&new_centers_locks);
	}
...
#pragma omp parallel for default(none) private(i, j, index) \
 shared(npoints, membership, feature, nfeatures, clusters, nclusters, new_centers_len, new_centers, new_centers_locks) \
 reduction(+:delta)
		// za svaki element
        for (i=0; i<npoints; i++) {
...
shared(nclusters, nfeatures, new_centers_len, clusters, new_centers) 
        for (i=0; i<nclusters; i++) {
            for (j=0; j<nfeatures; j++) {
