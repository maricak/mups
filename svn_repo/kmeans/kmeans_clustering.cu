...
__global__ void FloatReduction(float* cuda_delta_array, int n, float* cuda_delta_result_array) {
...
__global__ void updateNewCenters(int npoints, float* cuda_delta_array, int* cuda_membership, float* cuda_features, int nfeatures, float* cuda_clusters, int nclusters, int* cuda_new_centers_len, float* cuda_new_centers) {
	...
	if(global_id < npoints) {
		...
		for(int i = 0; i < nclusters; i++) {
			...
			for(int j = 0; j < nfeatures; j++) {
				cuda_new_centers[(i * nfeatures + j) * npoints + global_id] = 0;
			}
		}
		cuda_new_centers_len[index * npoints + global_id] = 1; // race condition
		for (int j = 0; j < nfeatures; j++)
			cuda_new_centers[(index * nfeatures + j) * npoints + global_id] += cuda_features[global_id * nfeatures + j];
	}
}
	...
	int *membership) /* out: [npoints] */
{
	...
	checkError(cudaMalloc(&cuda_new_centers, nclusters * nfeatures * npoints * sizeof(float)), __LINE__);
	float * cuda_new_centers_result;
	checkError(cudaMalloc(&cuda_new_centers_result, nclusters * nfeatures * gridDim.x * sizeof(float)), __LINE__);
	float * new_centers_result = (float*)malloc(nclusters * nfeatures * gridDim.x * sizeof(float));
	...
	*/
	...
	do {
		...
		FloatReduction <<< gridDim, blockDim >>>(cuda_delta_array, npoints, cuda_delta_result_array);
		checkError(cudaDeviceSynchronize(), __LINE__);
...
//	cuda_new_centers[(index * nfeatures + j) * npoints + global_id] += cuda_feature[global_id * nfeatures + j];

		for(int i = 0; i < nclusters; i++) {
			for(int j = 0; j < nfeatures; j++) {
				FloatReduction <<<gridDim, blockDim>>>(cuda_new_centers + (i * nfeatures + j) * npoints , npoints, cuda_new_centers_result + (i * nfeatures + j) * gridDim.x);
			
			}
		}
		checkError(cudaDeviceSynchronize(), __LINE__);

		checkError(cudaMemcpy(new_centers_result, cuda_new_centers_result, nclusters * nfeatures * gridDim.x * sizeof(float), cudaMemcpyDeviceToHost), __LINE__);
		for(int i = 0; i < nclusters; i++) {
			for(int j = 0; j < nfeatures; j++) {
				for(int k = 0; k < gridDim.x; k++) {
					new_centers[i][j] += new_centers_result[(i * nfeatures + j) * gridDim.x + k];
				}
			}
		}
		/* replace old cluster centers with new_centers */
	...
	checkError(cudaFree(cuda_delta_array), __LINE__);
	checkError(cudaFree(cuda_delta_result_array), __LINE__);
	checkError(cudaFree(cuda_new_centers_len), __LINE__);
	checkError(cudaFree(cuda_new_centers_len_result), __LINE__);
	checkError(cudaFree(cuda_new_centers), __LINE__);
	checkError(cudaFree(cuda_new_centers_result), __LINE__);
	
	free(new_centers[0]);
	free(new_centers);
	free(new_centers_len);

	free(delta_result_array);
	free(new_centers_len_result);
	free(new_centers_result);
