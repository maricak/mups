...
void basicSgemm_par(int rank, int size, char transa, char transb, int m, int n, int k, float alpha, float *A, int lda, float *B, int ldb, float beta, float *C, int ldc)
{
  ...
  MPI_Type_vector(k, CHUNK, m, MPI_FLOAT, &Arow);
  ...
  MPI_Type_vector(n, CHUNK, m, MPI_FLOAT, &Crow);
  ...
  if (rank == MASTER)
  {
    int count = m / CHUNK; // broj redova matice C
    ...
    for (sendCount = 0; sendCount < count && sendCount + 1 < size; sendCount++)
    {
      ...
      //int row = sendCount / n;
      int row = sendCount * CHUNK;
      //int col = sendCount % n;

      MPI_Request request;

      // salje se redni broj reda
      //printf("SLanje reda %d procesu %d\n", row, sendCount + 1);
...
*/
  ...
  { // SLAVE
    ...
    Abuff = new float[k * CHUNK]; // red matrice A
    //Bbuff = new float[k]; // kolona
    Bbuff = B;                    // matrica B
    Cbuff = new float[n * CHUNK]; // red matice C
                                  /*
...
*/
    ...
    while (needed)
    {
      ...
      MPI_Recv(Abuff, k * CHUNK, MPI_FLOAT, MASTER, ROW_TAG, MPI_COMM_WORLD, 0);
      /*printf("SLAVE%d Primljen Abuff(%d)", rank, row);
      for (int i = 0; i < k*CHUNK; printf("%F ", Abuff[i++]))
        ;
      printf("\n");
*/
...
*/
      ...
      MPI_Send(Cbuff, n * CHUNK, MPI_FLOAT, MASTER, RESULT_TAG, MPI_COMM_WORLD);
